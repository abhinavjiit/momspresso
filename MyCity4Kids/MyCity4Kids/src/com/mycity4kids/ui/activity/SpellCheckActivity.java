...
public class SpellCheckActivity extends BaseActivity {

    private TextView mTextView;
    private Map<String, ArrayList<String>> suggestionsMap = new HashMap<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String bodyContent = getIntent().getStringExtra("bodyContent");
        mTextView.setText(bodyContent);
        ...
        call.enqueue(spellCheckReponseCallback);

    }

    private Callback<ResponseBody> spellCheckReponseCallback = new Callback<ResponseBody>() {
        @Override
            ...
            try {
                if (response.isSuccessful()) {
                    ...
                    for (int i = 0; i < flaggedArray.length(); i++) {
                        ...
                        suggestionsMap.put(flaggedArray.getJSONObject(i).getString("token"), suggestionList);
                        highlightString(flaggedArray.getJSONObject(i).getString("token"));
                    }
                }
        ...
        while (indexOfKeyword > 0) {
            ...
            spannableString.setSpan(new ClickableSpan() {
                ...
                public void onClick(@NonNull View widget) {
                    ...
                    showPopUpMenu(centerView, input);
                }
...
//Set the final text on TextView
    ...
    public void showPopUpMenu(View view, String token) {
        ...
        popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            public boolean onMenuItemClick(MenuItem item) {
                String updatedText = mTextView.getText().toString().replaceAll(token, item.getTitle().toString());
                Log.d("Updated TE", updatedText);
                suggestionsMap.remove(token);
                mTextView.setText(updatedText);
                for (Map.Entry<String, ArrayList<String>> entry : suggestionsMap.entrySet()) {
                    highlightString(entry.getKey());
                }
                return true;
            }
