...
class ShortStoryTopicsGridAdapter : BaseAdapter() {
    ...
    private var topicsData = ArrayList<Topics>()
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (view == null) {
            ...
            view = LayoutInflater.from(parent!!.context).inflate(
                R.layout.explore_topics_grid_item,
                parent,
                false
            )
        ...
        try {
            Picasso.get().load(topicsData[position].extraData[0].categoryBackImage.app).placeholder(
                R.drawable.default_article
            ).error(R.drawable.default_article).fit().into(viewHolder.tagsImageView)
        } catch (e: Exception) {
    ...
    fun setTopicsData(topicsData: ArrayList<Topics>) {
