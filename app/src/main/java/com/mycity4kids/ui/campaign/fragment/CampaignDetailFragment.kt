...
class CampaignDetailFragment : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        shareText.setOnClickListener {
            activity?.let { it1 ->
                ...
                if (shareIntent.resolveActivity(it1.packageManager) != null) {
                    it1.startActivity(shareIntent)
                }
            }
        }
    ...
    private fun fetchCampaignDetail() {
        BaseApplication.getInstance().retrofit.create(CampaignAPI::class.java)
            .getCampaignDetail(this!!.id!!, 2.0).subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(object : Observer<BaseResponseGeneric<CampaignDetailResult>> {
    ...
    private fun setResponseData(apiGetResponsee: CampaignDetailResult?) {
        ...
        Picasso.get().load(apiGetResponse!!.imageUrl).placeholder(R.drawable.default_article)
            .error(R.drawable.default_article).into(bannerImg)
...
//    }
        ...
        override fun onResponse(
            ...
            try {
                ...
                if (responseData!!.code == 200 && Constants.SUCCESS == responseData.status) {
                    ...
                    Toast.makeText(context, responseData.reason, Toast.LENGTH_SHORT)
                        .show()////////////////////////////////////////////////
                }
            } catch (e: Exception) {
                Crashlytics.logException(e)
    ...
    private fun fetchDefaultCampaign() {
        ...
        BaseApplication.getInstance().retrofit.create(CampaignAPI::class.java)
            .getDefaultCampaignDetail().subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(object : Observer<BaseResponseGeneric<CampaignDetailResult>> {
                override fun onComplete() {
                    removeProgressDialog()
    ...
    fun setDefaultCampaignValues() {
        ...
        Picasso.get().load(defaultapigetResponse!!.imageUrl).placeholder(R.drawable.default_article)
            .error(R.drawable.default_article).into(default_campaign_header)
            ...
            try {
                ...
                if (responseData!!.code == 200) {
                    if (Constants.SUCCESS == responseData.status) {
                        Toast.makeText(context, responseData.data.get(0).msg, Toast.LENGTH_SHORT)
                            .show()
    ...
    fun unapplyCampaignDialog() {
        ...
        activity?.let {
            ...
            yesBtn.setOnClickListener {
                var participateRequest = CampaignParticipate()
                participateRequest.user_id = userId
                participateRequest.campaign_id = id!!
                val retro = BaseApplication.getInstance().retrofit
                val campaignAPI = retro.create(CampaignAPI::class.java)
    ...
    private fun fetchForYou() {
        // showProgressDialog(resources.getString(R.string.please_wait))
        BaseApplication.getInstance().retrofit.create(CampaignAPI::class.java)
            .getForYouStatus(SharedPrefUtils.getUserDetailModel(BaseApplication.getAppContext()).getDynamoId())
            .subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
            .subscribe(object : Observer<BasicResponse> {
