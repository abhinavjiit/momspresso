...
class TrackerListAdapter(var context: Context, var trackerDataModel: ArrayList<TrackerDataModel>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        var view =
            LayoutInflater.from(parent.context).inflate(R.layout.tracker_list_index, parent, false)
        ...
        if (!trackerData.isNullOrEmpty()) {
            ...
            if (trackerDataModel != null) {
                ...
                if (position == 0) {
                    ...
                    holder.imageTop.setColorFilter(
                ...
                holder.textStatusName.text =
                    Constants.TrackerStatusMapping.findById(trackerDataModel.tracker_status)
                if (trackerDataModel.is_completed == 1) {
                    ...
                    setColorsAndImage(
                        ...
                        context,
                        holder.textDate,
                        holder.textStatusName
                    )
                    holder.textDateError.text = ""
                    if (Constants.TrackerStatusMapping.findById(trackerDataModel.tracker_status).equals(
                            context.resources.getString(R.string.tracker_activity_proof_submitted_rejected)
                        )) {
                        if (errorStatus) {
                            holder.textstatusError.text =
                                context.resources.getString(R.string.tracker_Activity_submit_corrected_proof)
                        } else {
                            holder.textstatusError.text = ""
                        }
                    } else {
                    ...
                    if (trackerDataModel.expected_time > 0) {
                        ...
                        holder.textDate.visibility = View.VISIBLE
                        holder.textDateError.text =
                            context.resources.getString(R.string.tracker_Activity_expected_date)
                    } else {
                        holder.textDate.visibility = View.GONE
                        holder.textDateError.text = ""
                    }
                    holder.imageStatus.setImageDrawable(context.getDrawable(R.drawable.ic_circle_svg))
                    holder.imageStatus.setColorFilter(
                        ContextCompat.getColor(
                            context,
                            R.color.campaign_expired
                        ), android.graphics.PorterDuff.Mode.SRC_IN
                    )
                    holder.textstatusError.text = ""
...
fun setColorsAndImage(
    statusCode: String,
    imageStatus: ImageView,
    context: Context,
    textDate: TextView,
    textStatusName: TextView
) {
    when {
        "APPROVED".equals(statusCode, true) -> {
            imageStatus.setColorFilter(
                ContextCompat.getColor(context, R.color.campaign_subscribed),
                android.graphics.PorterDuff.Mode.SRC_IN
            )
        ...
        "APPLIED".equals(statusCode, true) -> {
            imageStatus.setColorFilter(
                ContextCompat.getColor(context, R.color.campaign_applied_bg),
                android.graphics.PorterDuff.Mode.SRC_IN
            )
        ...
        "PROOFS APPROVED".equals(statusCode, true) -> {
            imageStatus.setColorFilter(
                ContextCompat.getColor(
                    context,
                    R.color.campaign_proof_approval
                ), android.graphics.PorterDuff.Mode.SRC_IN
            )
        ...
        "PROOFS SUBMITTED REJECTED".equals(statusCode, true) -> {
            imageStatus.setColorFilter(
                ContextCompat.getColor(
                    context,
                    R.color.campaign_proof_reject_bg
                ), android.graphics.PorterDuff.Mode.SRC_IN
            )
        ...
        "PAYMENT IN PROCESS".equals(statusCode, true) -> {
            imageStatus.setColorFilter(
                ContextCompat.getColor(
                    context,
                    R.color.campaign_payment_in_process
                ), android.graphics.PorterDuff.Mode.SRC_IN
            )
        ...
        "PROOFS SUBMITTED".equals(statusCode, true) -> {
            imageStatus.setColorFilter(
                ContextCompat.getColor(
                    context,
                    R.color.campaign_proof_submitted
                ), android.graphics.PorterDuff.Mode.SRC_IN
            )
