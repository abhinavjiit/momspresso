...
public class MentionsEditText extends AppCompatEditText implements TokenSource {
    ...
    private List<MentionWatcher> mentionWatchers = new ArrayList<>();
    private List<TextWatcher> externalTextWatchers = new ArrayList<>();
    private final MyWatcher internalTextWatcher = new MyWatcher();
    private boolean blockCompletion = false;
    private boolean isWatchingText = false;
    private boolean avoidPrefixOnTap = false;
    @Nullable
    private String avoidedPrefix;
    ...
    private MentionSpanConfig parseMentionSpanConfigFromAttributes(@Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.getTheme()
                .obtainStyledAttributes(attrs, R.styleable.MentionsEditText, defStyleAttr, 0);
        ...
        @ColorInt int selectedTextColor = attributes
                .getColor(R.styleable.MentionsEditText_selectedMentionTextColor, -1);
        builder.setSelectedMentionTextColor(selectedTextColor);
        @ColorInt int selectedBgColor = attributes
                .getColor(R.styleable.MentionsEditText_selectedMentionTextBackgroundColor, -1);
    ...
    public String getCurrentTokenString() {
        ...
        int start = tokenizer.findTokenStart(text, cursor);
        int end = tokenizer.findTokenEnd(text, cursor);
    ...
    public QueryToken getQueryTokenIfValid() {
        ...
        boolean isExplicit = tokenizer.isExplicitChar(tokenString.charAt(0));
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        boolean superResult;
        if (Build.VERSION.SDK_INT == Build.VERSION_CODES.M && event.getActionMasked() == MotionEvent.ACTION_UP) {
            try {
        ...
        if (avoidPrefixOnTap
                && suggestionsVisibilityManager != null
                && suggestionsVisibilityManager.isDisplayingSuggestions()) {
    ...
    /**
     * Copy the text between start and end in clipboard. If no span is present, text is saved as plain text but if span
     * is present save it in Clipboard using intent.
     */
    ...
    /**
     * Paste clipboard content between min and max positions. If clipboard content contain the MentionSpan, set the span
     * in copied text.
     */
        ...
        if (clip != null) {
            for (int i = 0; i < clip.getItemCount(); i++) {
                ...
                for (int j = 0; j < parcelables.length; j++) {
                    ...
                    s.setSpan(mentionSpan, spanStart[j], spanStart[j] + mentionSpan.getDisplayString().length(),
    ...
    /**
     * Gets the {@link MentionSpan} from the {@link MentionsEditText} that was tapped. Note: Almost all of this code is
     * taken directly from the Android source code, see: {@link LinkMovementMethod#onTouchEvent(TextView, Spannable,
     * MotionEvent)}
     *
     * @param event the given (@link MotionEvent}
    ...
    /**
     * Method to handle the cursor changing positions. Returns true if handled, false if it should be passed to the
     * super method.
     *
    ...
    private class MyWatcher implements TextWatcher {
        ...
        public void onTextChanged(CharSequence text, int start, int before, int count) {
            if (blockCompletion || !(text instanceof Editable) || getTokenizer() == null) {
                return;
            }
        ...
        /**
         * Notify external text watchers that the text is about to change. See {@link
         * TextWatcher#beforeTextChanged(CharSequence, int, int, int)}.
         */
        private void sendBeforeTextChanged(CharSequence text, int start, int before, int after) {
            final List<TextWatcher> list = externalTextWatchers;
        ...
        /**
         * Notify external text watchers that the text is changing. See {@link TextWatcher#onTextChanged(CharSequence,
         * int, int, int)}.
         */
        ...
        /**
         * Notify external text watchers that the text has changed. See {@link TextWatcher#afterTextChanged(Editable)}.
         */
    ...
    /**
     * Marks a span for deletion later if necessary by checking if the last character in a MentionSpan is deleted by
     * this change. If so, mark the span to be deleted later when {@link #ensureMentionSpanIntegrity(Editable)} is
     * called in {@link #handleTextChanged()}.
     *
     ...
     * is to remove any MentionSpans that could cause issues while the user is changing text. Note: The MentionSpans are
     * added again in {@link #replacePlaceholdersWithCorrespondingMentionSpans(Editable)}
     *
    ...
    private int findStartOfWord(@NonNull CharSequence text, int index) {
        int wordStart = index;
        while (wordStart > 0 && tokenizer != null && !tokenizer.isWordBreakingChar(text.charAt(wordStart - 1))) {
            wordStart--;
        }
        return wordStart;
    }
    ...
    /**
     * Removes any {@link com.mycity4kids.tagging.ui.MentionsEditText.DeleteSpan}s and the text within them from the
     * given text.
     *
    ...
    /**
     * Replaces any {@link com.mycity4kids.tagging.ui.MentionsEditText.PlaceholderSpan} within the given text with the
     * {@link MentionSpan} it contains.Note: These PlaceholderSpans are added in {@link
     * #replaceMentionSpansWithPlaceholdersAsNecessary(CharSequence)}
     *
    ...
    private void replacePlaceholdersWithCorrespondingMentionSpans(@NonNull Editable text) {
        ...
        for (PlaceholderSpan span : tempSpans) {
            ...
            text.setSpan(span.holder, spanStart, spanStart + mentionDisplayString.length(),
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            text.removeSpan(span);
        }
    }
    ...
    /**
     * Ensures that the text within each {@link MentionSpan} in the {@link Editable} correctly matches what it should be
     * outputting. If not, replace it with the correct value.
     *
            ...
            switch (displayMode) {
                ...
                case FULL:
                    ...
                    if (!name.contentEquals(spanText) && start >= 0 && start < end && end <= text.length()) {
                        ...
                        if (mentionWatchers.size() > 0 && displayMode == Mentionable.MentionDisplayMode.PARTIAL) {
                ...
                default:
                    ...
                    boolean hasListeners = mentionWatchers.size() > 0;
    ...
    /**
     * Called after the {@link Editable} text within the {@link EditText} has been changed. Note that editing text in
     * this function is guaranteed to be safe and not cause an infinite loop.
     */
    private void handleTextChanged() {
        // Ignore requests if the last word in keywords is prefixed by the currently avoided prefix
        if (avoidedPrefix != null) {
            ...
            if (lastKeyword.startsWith(avoidedPrefix)) {
                return;
            } else {
        ...
        if (queryToken != null && queryTokenReceiver != null) {
            // Valid token, so send query to the app for processing
            queryTokenReceiver.onQueryReceived(queryToken);
        } else {
    ...
    public String getCurrentKeywordsString() {
        ...
        if (keywordsString.length() > 0 && tokenizer.isExplicitChar(keywordsString.charAt(0))) {
    ...
    public boolean isCurrentlyExplicit() {
        ...
        return tokenString.length() > 0 && tokenizer != null && tokenizer.isExplicitChar(tokenString.charAt(0));
    }
    ...
    /**
     * Allows a class to watch for text changes. Note that adding this class to itself will add it to the super class.
     * Other instances of {@link TextWatcher} will be notified by this class as appropriate (helps prevent infinite
     * loops when text keeps changing).
     *
     * @param watcher the {@link TextWatcher} to add
     */
    @Override
    public void addTextChangedListener(@NonNull TextWatcher watcher) {
        if (watcher == internalTextWatcher) {
            if (!isWatchingText) {
                super.addTextChangedListener(internalTextWatcher);
                isWatchingText = true;
            }
        } else {
            externalTextWatchers.add(watcher);
        }
    }

    /**
     * Removes a {@link TextWatcher} from this class. Note that this function servers as the counterpart to {@link
    ...
    public void removeTextChangedListener(@NonNull TextWatcher watcher) {
        if (watcher == internalTextWatcher) {
            if (isWatchingText) {
                super.removeTextChangedListener(internalTextWatcher);
                isWatchingText = false;
            }
        } else {
            // Other watchers are added
            externalTextWatchers.remove(watcher);
        }
    }
    ...
    public void addMentionWatcher(@NonNull MentionWatcher watcher) {
        if (!mentionWatchers.contains(watcher)) {
            mentionWatchers.add(watcher);
        }
    }
    ...
    public void removeMentionWatcher(@NonNull MentionWatcher watcher) {
        mentionWatchers.remove(watcher);
    }
    ...
    private void notifyMentionAddedWatchers(@NonNull Mentionable mention, @NonNull String text, int start, int end) {
        for (MentionWatcher watcher : mentionWatchers) {
    ...
    private void notifyMentionPartiallyDeletedWatchers(@NonNull Mentionable mention, @NonNull String text, int start,
            int end) {
        for (MentionWatcher watcher : mentionWatchers) {
    ...
    /**
     * Custom {@link MovementMethod} for this class used to override specific behavior in {@link
    ...
    /**
     * Sets the tokenizer used by this class. The tokenizer determines how {@link QueryToken} objects are generated.
     *
     * @param tokenizer the {@link Tokenizer} to use
     */
    ...
    /**
     * Sets the receiver of query tokens used by this class. The query token receiver will use the tokens to generate
     * suggestions, which can then be inserted back into this edit text.
     *
     ...
     */
    public void setSuggestionsVisibilityManager(
            @Nullable final SuggestionsVisibilityManager suggestionsVisibilityManager) {
    ...
    public void setAvoidedPrefix(@Nullable String prefix) {
        avoidedPrefix = prefix;
    }

    /**
     * Determines whether the edit text should avoid the current prefix if the user taps on it while it is displaying
    ...
    public void setAvoidPrefixOnTap(boolean avoidPrefixOnTap) {
        this.avoidPrefixOnTap = avoidPrefixOnTap;
    }
    ...
    /**
     * Simple implementation of the {@link com.mycity4kids.tagging.ui.MentionsEditText.MentionWatcher} interface if you
     * do not want to implement all methods.
     */
    @SuppressWarnings("unused")
