...
package com.mycity4kids.tagging.tokenization.impl;
...
public class WordTokenizer implements Tokenizer {
    ...
    private final WordTokenizerConfig wordTokenizerConfig;
    ...
    public int findTokenStart(final @NonNull Spanned text, final int cursor) {
        ...
        if (isExplicit(text, cursor)) {
            ...
            int maxNumKeywords = wordTokenizerConfig.MAX_NUM_KEYWORDS;
    ...
    public boolean isValidMention(final @NonNull Spanned text, final int start, final int end) {
        ...
        final int threshold = wordTokenizerConfig.THRESHOLD;
    ...
    public boolean isExplicitChar(final char c) {
        final String explicitChars = wordTokenizerConfig.EXPLICIT_CHARS;
    ...
    public boolean isWordBreakingChar(final char c) {
        final String wordBreakChars = wordTokenizerConfig.WORD_BREAK_CHARS;
    ...
    public char getExplicitChar(final @NonNull CharSequence text, final int cursor) {
        ...
        while (i >= start) {
            ...
            if (isExplicitChar(currentChar)) {
                ...
                if (numWordBreakingCharsSeen == wordTokenizerConfig.MAX_NUM_KEYWORDS) {
    ...
    protected int getSearchStartIndex(final @NonNull Spanned text, int cursor) {
        ...
        if (textString.contains(wordTokenizerConfig.LINE_SEPARATOR)) {
            lineStartIndex = textString.lastIndexOf(wordTokenizerConfig.LINE_SEPARATOR) + 1;
        }
    ...
    protected int getSearchEndIndex(final @NonNull Spanned text, int cursor) {
        ...
        if (textString.contains(wordTokenizerConfig.LINE_SEPARATOR)) {
            lineEndIndex = cursor + textString.indexOf(wordTokenizerConfig.LINE_SEPARATOR);
        }
