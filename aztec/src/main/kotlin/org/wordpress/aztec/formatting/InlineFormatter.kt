...
class InlineFormatter(editor: AztecText, val codeStyle: CodeStyle) : AztecFormatter(editor) {
    ...
    fun handleInlineStyling(textChangedEvent: TextChangedEvent, context: Context) {
        try {
            ...
            Toast.makeText(context, "exception1", Toast.LENGTH_SHORT)
        }
    }

    private fun clearInlineStyles(start: Int, end: Int, ignoreSelectedStyles: Boolean,context: Context) {
        try {
        ...
        editableText.getSpans(newStart, end, IAztecInlineSpan::class.java).forEach {
            ...
            Toast.makeText(context, "exception2", Toast.LENGTH_SHORT)
        }
    }
